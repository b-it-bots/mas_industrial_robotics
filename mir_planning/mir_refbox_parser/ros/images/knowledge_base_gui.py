...
class VisualizeWorldModel:
    def __init__(self):
        ...
        self.canvas.create_image(
            self.locations["SH-01"][0],
            self.locations["SH-01"][1],
            anchor=tk.NW,
            image=rect,
        )
        self.canvas.create_image(
            self.locations["SH-02"][0],
            self.locations["SH-02"][1],
            anchor=tk.NW,
            image=rect,
        )
        self.canvas.create_image(
            self.locations["SH-03"][0],
            self.locations["SH-03"][1],
            anchor=tk.NW,
            image=rect,
        )
        self.canvas.create_image(
            self.locations["SH-04"][0],
            self.locations["SH-04"][1],
            anchor=tk.NW,
            image=rect,
        )
        self.canvas.create_image(
            self.locations["WS-01"][0],
            self.locations["WS-01"][1],
            anchor=tk.NW,
            image=rect,
        )
        self.canvas.create_image(
            self.locations["WS-03"][0],
            self.locations["WS-03"][1],
            anchor=tk.NW,
            image=rect,
        )
        self.canvas.create_image(
            self.locations["WS-05"][0],
            self.locations["WS-05"][1],
            anchor=tk.NW,
            image=rect,
        )
        self.canvas.create_image(
            self.locations["WS-06"][0],
            self.locations["WS-06"][1],
            anchor=tk.NW,
            image=rect,
        )
        self.canvas.create_image(
            self.locations["WS-07"][0],
            self.locations["WS-07"][1],
            anchor=tk.NW,
            image=rect,
        )
        self.canvas.create_image(
            self.locations["CB-01"][0],
            self.locations["CB-01"][1],
            anchor=tk.NW,
            image=conveyor,
        )
        # self.canvas.create_image(self.locations['entry'][0],self.locations['entry'][1], anchor=tk.NW, image=entry)
        self.canvas.create_image(
            self.locations["yb"][0], self.locations["yb"][1], anchor=tk.NW, image=youbot
        )
    ...
    def query_database_facts(self):
        ...
        for key, loc in self.locations.iteritems():
            self.objects_on_location[key] = [""]
        try:
            model_client = rospy.ServiceProxy(
    ...
    def update_text(self):
        ...
        for key, loc in self.locations.iteritems():
            ...
            self.canvas.itemconfig(
                loc[6],
                text=("\n".join(self.goals_from_knowledge_base[key])),
                anchor=tk.NW,
                fill="red",
            )
