...
class RefboxParser(object):
    ...
    def __init__(self):
        ...
        # event in subscriber
        rospy.Subscriber(
            "~event_in", std_msgs.msg.String, self.eventInCallBack, queue_size=10
        )
        ...
        rospy.Subscriber(
            "/rockin/marker_pose",
            geometry_msgs.msg.PoseStamped,
            self.markerPoseCallBack,
            queue_size=1,
        )

        # For benchmarking
        self.pub_robot_pose = rospy.Publisher(
            "/rockin/robot_pose_waypoint", geometry_msgs.msg.PoseStamped, queue_size=1
        )
        self.pub_marker_pose = rospy.Publisher(
            "/rockin/marker_pose_waypoint", geometry_msgs.msg.PoseStamped, queue_size=1
        )

        # For logging
        self.pub_logging_status = rospy.Publisher(
            "/robot_example_ros/logging_status",
            atwork_ros_msgs.msg.LoggingStatus,
            queue_size=1,
        )
        self.pub_logger = rospy.Publisher(
            "/mcr_tools/rosbag_recorder/event_in", std_msgs.msg.String, queue_size=1
        )
        ...
        if bnt:
            rospy.loginfo(
                "Running BNT from refbox. Not waiting for the database world model"
            )
        else:
            ...
            try:
                predicates_client = rospy.ServiceProxy(
            ...
            try:
                type_client = rospy.ServiceProxy(
                    "/rosplan_knowledge_base/domain/types", GetDomainTypeService
                )
                resp = type_client()
    ...
    def eventInCallBack(self, msg):
        if msg.data == "e_trigger":
            rospy.loginfo(
                "Received trigger updating knowledge base based on order and invetory data"
            )
            self.start_inventory_flag = True
            self.start_order_flag = True
            ...
            if attribute:
                for uploaded_instance in object_instance_list:
                    self.load_fact_message(
                        attribute, uploaded_instance, item, instance_type_identifyer
                    )
    ...
    def msg_to_instance_name(self, item, domain_name, input_count=1):
        ...
        if domain_name == "location":
            text = self.location_type[item.location.type.data] + str(
                item.location.instance_id.data
            ).zfill(2)

        if domain_name == "destination":
            text = self.location_type[item.destination.type.data] + str(
                item.destination.instance_id.data
            ).zfill(2)
        return text
