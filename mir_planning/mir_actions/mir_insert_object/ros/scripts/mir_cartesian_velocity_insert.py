...
class cartesian_velocity_insert():
    ...
    def __init__(self):
        ...
        self.object_pose_sub = rospy.Subscriber('/pregrasp_planner_node/pose_samples', PoseArray, self.object_pose_cb)
        self.feedforward_gain = 4
        self.feedback_gain = 3
        self.debug_pose_pub = rospy.Publisher('/insert_object_workaround/modified_pose_debug', PoseStamped, queue_size=11)
        """
        ...
        self.event_out = rospy.Publisher("/insert_object_workaroud/event_out", String, queue_size=1)
        self.start_end = None
        self.goal = None
        self.z_offset = 0.03
    ...
    def object_pose_cb(self, data):
        ...
        while not rospy.is_shutdown():
            try:
                pose_stamped_1 = self.tf_listener.transformPose('/arm_link_0', pose_stamped_)
                break
            except:
                continue
        self.goal = [pose_stamped_1.pose.position.x,
        ...
        while not rospy.is_shutdown():
            try:
                pose_stamped_2 = self.tf_listener.transformPose('/base_link', pose_stamped_1)
                break
            except:
                continue
        self.debug_pose_pub.publish(pose_stamped_2)
        self.goal = [pose_stamped_2.pose.position.x,
    ...
    def move_arm_to_goal(self):
        ...
        while not rospy.is_shutdown() and self.event != 'e_stop':
            try:
                (current_pos,rot) = self.tf_listener.lookupTransform('/base_link', '/arm_link_5', rospy.Time(0))
                break
            except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
                continue
        
        path_x = np.linspace(current_pos[0], goal_point[0], self.number_of_sampling_points)
        path_y = np.linspace(current_pos[1], goal_point[1], self.number_of_sampling_points)
        path_z = np.linspace(current_pos[2], goal_point[2], self.number_of_sampling_points)
