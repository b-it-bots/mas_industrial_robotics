...
class ppt_wiggle_arm(smach.State):
    ...
    def execute_arm(self, joint_number, wiggle_offset ):
        ...
        joint_values[joint_number] = self.joint_values_static[joint_number] + wiggle_offset
        try:
    ...
    def execute(self, userdata):
        ...
        if (np.allclose(self.yaw,0)):
            ...
            joint_number_to_change = 0; wiggle_offset = -0.12;
            self.execute_arm(joint_number_to_change, wiggle_offset )
            #go left 
            joint_number_to_change = 0; wiggle_offset = 0.12;
            self.execute_arm(joint_number_to_change, wiggle_offset )
            #go center again
            joint_number_to_change = 0; wiggle_offset = 0.0;
            self.execute_arm(joint_number_to_change, wiggle_offset )
            #wiggle forward
            joint_number_to_change = 3; wiggle_offset = -0.12;
            self.execute_arm(joint_number_to_change, wiggle_offset )
            #go left 
            joint_number_to_change = 3; wiggle_offset = 0.12;
            self.execute_arm(joint_number_to_change, wiggle_offset )
            #go center again
            joint_number_to_change = 3; wiggle_offset = 0.0;
            self.execute_arm(joint_number_to_change, wiggle_offset )
        else :
            #wiggle forward
            joint_number_to_change = 3; wiggle_offset = -0.12;
