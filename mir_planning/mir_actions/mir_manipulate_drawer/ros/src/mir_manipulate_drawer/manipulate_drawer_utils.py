    ...
    def calc_pose_for_dbc_for_param(self, pose_in, param_name="default"):
        ...
        if pose_in is None:
            ...
            dbc_target_pose.pose.position.x = (
                pose_in.pose.position.x + dbc_param["x_offset"]
            )
...
class SendPoseToDBC(smach.State):
    def __init__(self, param_name="default"):
        smach.State.__init__(self, outcomes=["succeeded"], input_keys=["handle_pose"])
        self._dbc_pose_pub = rospy.Publisher(
            "/mcr_navigation/direct_base_controller/input_pose",
            PoseStamped,
            queue_size=10,
    ...
    def execute(self, userdata):
        dbc_pose = self.utils.calc_pose_for_dbc_for_param(
