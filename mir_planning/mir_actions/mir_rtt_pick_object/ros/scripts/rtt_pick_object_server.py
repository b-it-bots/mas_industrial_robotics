...
class WaitForObject(smach.State):
    ...
    def execute(self, userdata):
        ...
        if rospy.Time.now().to_sec() > userdata.time - 0.80:   #0.77 prev 
            return "failed"
        while rospy.Time.now().to_sec() < userdata.time - 0.80: # This is the time to close the gripper for picking. The higher the time the earlier the gripper will close
            rospy.sleep(0.01)
        return "succeeded"
