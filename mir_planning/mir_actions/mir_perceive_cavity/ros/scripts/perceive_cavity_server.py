...
class PopulateResultWithCavities(smach.State):
    def __init__(self):
        smach.State.__init__(self,  outcomes=['succeeded'],
                                    input_keys=['goal'],
                                    output_keys=['feedback', 'result'])
        self.cavities_sub = rospy.Subscriber('/mcr_perception/cavity_finder/output/cavities_list',
...
def main():
    ...
    with sm:
        ...
        smach.StateMachine.add('PUBLISH_REFERENCE_FRAME_FOR_WBC', gbs.send_event(
        ...
        smach.StateMachine.add('START_POSE_SELECTOR', gbs.send_event(
                    [('/mcr_perception/cavity_pose_selector/event_in', 'e_start')]),
                transitions={'success':'LOOK_AROUND'})


        # move arm to selected pose
        smach.StateMachine.add('LOOK_AROUND', gms.move_arm('look_at_workspace_from_near', blocking=True),
        ...
        smach.StateMachine.add('RECOGNIZE_CAVITIES', gps.find_cavities(retries=1),
            transitions={'cavities_found': 'POPULATE_RESULT_WITH_CAVITIES',
                        'no_cavities_found':'OVERALL_FAILED'})

        # populate action server result with perceived objects
        smach.StateMachine.add('POPULATE_RESULT_WITH_CAVITIES', PopulateResultWithCavities(), 
                transitions={'succeeded':'OVERALL_SUCCESS'})

    # smach viewer
    if rospy.get_param('~viewer_enabled', False):
        sis = IntrospectionServer('perceive_cavity_smach_viewer', sm, '/PERCEIVE_CAVITY_SMACH_VIEWER')
        sis.start()
