...
{
    // get domain name
    ros::ServiceClient getNameClient = nh_.serviceClient<rosplan_knowledge_msgs::GetDomainNameService>(domain_name_service_);
    rosplan_knowledge_msgs::GetDomainNameService name_srv;
    if (!getNameClient.call(name_srv)) {
        ROS_ERROR("[mir_pddl_problem_generator] Failed to call service %s", domain_name_service_.c_str());
        return false;
    }
...
{
    ros::ServiceClient getTypesClient = nh_.serviceClient<rosplan_knowledge_msgs::GetDomainTypeService>(domain_type_service_);
    ros::ServiceClient getInstancesClient = nh_.serviceClient<rosplan_knowledge_msgs::GetInstanceService>(state_instance_service_);
    ...
    if (!getTypesClient.call(type_srv)) {
        ROS_ERROR("[mir_pddl_problem_generator] Failed to call service %s", domain_type_service_.c_str());
    }
...
{
    ros::ServiceClient getDomainPropsClient = nh_.serviceClient<rosplan_knowledge_msgs::GetDomainAttributeService>(domain_predicate_service_);
    ros::ServiceClient getPropsClient = nh_.serviceClient<rosplan_knowledge_msgs::GetAttributeService>(state_proposition_service_);
    ...
    {
        ROS_ERROR("[mir_pddl_problem_generator] Failed to call service %s", domain_predicate_service_.c_str());
    }
    else
    {
        std::vector<rosplan_knowledge_msgs::DomainFormula>::iterator ait = domain_attr_srv.response.items.begin();
        for(; ait != domain_attr_srv.response.items.end(); ait++)
        {
            ...
            {
                ROS_ERROR("[mir_pddl_problem_generator] Failed to call service %s: %s", state_proposition_service_.c_str(), attr_srv.request.predicate_name.c_str());
            }
            else
            {
...
{
    ros::ServiceClient getCurrentGoalsClient = nh_.serviceClient<rosplan_knowledge_msgs::GetAttributeService>(state_goal_service_);
    ...
    {
        ROS_ERROR("[mir_pddl_problem_generator] Failed to call service %s", state_goal_service_.c_str());
    }
    else
    {
        for (size_t i = 0; i < current_goal_srv.response.attributes.size(); i++)
        {
            ...
            if (attr.knowledge_type != rosplan_knowledge_msgs::KnowledgeItem::FACT) { continue; }
        ...
        while (true)
        {
            ...
            if (goal_counter >= max_goals_) { break; }
