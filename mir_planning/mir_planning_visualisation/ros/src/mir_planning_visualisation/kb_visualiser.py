...
class KnowledgeBaseVisualiser(object):
    ...
    def __init__(self):
        ...
        self._kb_server_facts_topic = rospy.get_param(
            "~kb_server_facts_topic", "~facts"
        )
        self._kb_server_goals_topic = rospy.get_param(
            "~kb_server_goals_topic", "~goals"
        )
    ...
    def _get_markers_from_data(self, data):
        ...
        for ws_name, obj_list in data["goal_obj_on_ws"].iteritems():
            ...
            for marker in obj_markers:
                ...
                marker.scale.x = (
                    marker.scale.y
                ) = marker.scale.z = self._goal_obj_config["scale"]
