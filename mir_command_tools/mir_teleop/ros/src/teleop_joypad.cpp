...
TeleOpJoypad::TeleOpJoypad(ros::NodeHandle &nh) {
  ...
  if (this->getArmParameter()) {
    ROS_INFO(
        "Arm joint limit parameters available. Joint space control: ACTIVE");

    sub_joint_states_ = nh_->subscribe<sensor_msgs::JointState>(
        "/joint_states", 1, &TeleOpJoypad::cbJointStates, this);
  ...
  sub_joypad_ = nh_->subscribe<sensor_msgs::Joy>("/joy", 1,
                                                 &TeleOpJoypad::cbJoypad, this);
  ...
  pub_gripper_command_ = nh_->advertise<mcr_manipulation_msgs::GripperCommand>(
      "gripper_command", 1);

  srv_base_motors_on_ =
      nh_->serviceClient<std_srvs::Empty>("/base/switchOnMotors");
  srv_base_motors_off_ =
      nh_->serviceClient<std_srvs::Empty>("/base/switchOffMotors");
  srv_arm_motors_on_ =
      nh_->serviceClient<std_srvs::Empty>("/arm_1/switchOnMotors");
  srv_arm_motors_off_ =
      nh_->serviceClient<std_srvs::Empty>("/arm_1/switchOffMotors");
    ...
    for (int j = 0; j < name_list.size(); ++j) {
      if (i == 0) {
        if (name_list[j] == "deadman")
          ...
          ROS_WARN_STREAM("button name <<"
                          << name_list[j] << ">> in yaml but not used in node");
      }
        ...
        if (name_list[j] == "base_linear_x")
          ...
          ROS_WARN_STREAM("axes name <<" << name_list[j]
                                         << ">> in yaml but not used in node");
      }
    }
  }
...
bool TeleOpJoypad::getArmParameter() {
  ...
  ros::param::param<double>("~soft_joint_limit_threshold",
                            soft_joint_limit_threshold_, 0.05);
  ...
  if (nh_->getParam("/arm_1/arm_controller/joints", param_list)) {
    ...
    for (unsigned int i = 0; i < arm_joint_names_.size(); ++i) {
      ...
      nh_->getParam(
          "/arm_1/arm_controller/limits/" + arm_joint_names_[i] + "/min",
          limit.min_position);
      nh_->getParam(
          "/arm_1/arm_controller/limits/" + arm_joint_names_[i] + "/max",
          limit.max_position);
...
void TeleOpJoypad::cbDynamicReconfigure(mir_teleop::TeleopJoypadConfig &config,
                                        uint32_t level) {
  teleop_config_ = config;
  arm_cart_zero_vel_.header.frame_id =
      teleop_config_.arm_cartesian_reference_link;
}

void TeleOpJoypad::cbJointStates(
    const sensor_msgs::JointState::ConstPtr &state_msg) {
  current_joint_states_ = *state_msg;
}

bool TeleOpJoypad::switchMotorsOnOff(std::string component_name,
                                     std::string state) {
  ...
  if (component_name == "arm" && state == "ON")
    ...
    ROS_ERROR_STREAM("component <<" << component_name << ">> or state <<"
                                    << state << ">> not known.");
    return false;
  }
  ...
  if (srv_client->call(empty)) {
    ...
    ROS_ERROR_STREAM("Could not turn " << state << " " << component_name
                                       << " motors");
    return false;
  }
...
bool TeleOpJoypad::reconnect() {
  ...
  if (srv_reconnect.call(empty)) {
    ROS_INFO_STREAM("Call to serivce: " << srv_reconnect.getService()
                                        << " successful.");
    return true;
  }
...
void TeleOpJoypad::setSingleArmJointVel(double motor_vel,
...
void TeleOpJoypad::printArmJointStates() {
  ...
  std::cout << "] \t # current arm joint values (" << joint_name_list << ")"
            << std::endl;
}
    ...
    for (unsigned int j = 0; j < current_joint_states_.name.size(); ++j) {
      if (current_joint_states_.name[j] == arm_joint_limits_[i].joint_name) {
        if (((current_joint_states_.position[j] <
              (arm_joint_limits_[i].min_position +
               soft_joint_limit_threshold_)) &&
             (arm_vel_.velocities[j].value < 0)) ||
            ((current_joint_states_.position[j] >
              (arm_joint_limits_[i].max_position -
               soft_joint_limit_threshold_)) &&
             (arm_vel_.velocities[j].value > 0))) {
...
void TeleOpJoypad::cbJoypad(const sensor_msgs::Joy::ConstPtr &command) {
  ...
  if (static_cast<bool>(command->buttons[button_index_deadman_])) {
    speed_factor_ =
        !static_cast<bool>(command->buttons[button_index_run_]) ? 0.5 : 1.0;
    ...
    if (static_cast<bool>(command->buttons[button_index_arm_cart_])) {
      arm_cart_vel_.header.frame_id =
          teleop_config_.arm_cartesian_reference_link;
      arm_cart_vel_.twist.linear.x = command->axes[axes_index_arm_linear_x_] *
                                     arm_cart_factor_ * speed_factor_;
      arm_cart_vel_.twist.linear.y = command->axes[axes_index_arm_linear_y_] *
                                     arm_cart_factor_ * speed_factor_;
      arm_cart_vel_.twist.linear.z = command->axes[axes_index_arm_linear_z_] *
                                     arm_cart_factor_ * speed_factor_;

      arm_cart_vel_.twist.angular.x = command->axes[axes_index_arm_angular_x_] *
                                      arm_cart_factor_ * speed_factor_;
      arm_cart_vel_.twist.angular.y = command->axes[axes_index_arm_angular_y_] *
                                      arm_cart_factor_ * speed_factor_;
      arm_cart_vel_.twist.angular.z = command->axes[axes_index_arm_angular_z_] *
                                      arm_cart_factor_ * speed_factor_;
    } else {
      base_cart_vel_.linear.x = command->axes[axes_index_base_linear_x_] *
                                base_cart_factor_.linear.x * speed_factor_;
      base_cart_vel_.linear.y = command->axes[axes_index_base_linear_y_] *
                                base_cart_factor_.linear.y * speed_factor_;
      base_cart_vel_.angular.z = command->axes[axes_index_base_angular_z_] *
                                 base_cart_factor_.angular.z * speed_factor_;
    ...
    if (is_joint_space_ctrl_active_) {
      ...
      if (static_cast<bool>(command->buttons[button_index_arm_joint_1_2_])) {
        arm_vel_.velocities[0].value =
            command->axes[axes_index_arm_joint_axes_1_] * arm_max_vel_ *
            speed_factor_ * (-1.0);
        arm_vel_.velocities[1].value =
            command->axes[axes_index_arm_joint_axes_2_] * arm_max_vel_ *
            speed_factor_;
      } else if (static_cast<bool>(
                     command->buttons[button_index_arm_joint_3_4_])) {
  ...
  button_deadman_pressed_prev_ =
      static_cast<bool>(command->buttons[button_index_deadman_]);
  button_gripper_pressed_prev_ =
      static_cast<bool>(command->buttons[button_index_gripper_]);
