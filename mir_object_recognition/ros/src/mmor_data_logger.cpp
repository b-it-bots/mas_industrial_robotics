...
public:
    DataLogger(): Node("data_logger")
    {
        ...
        // raw_cv_image_subscriber_ = this->create_subscription<sensor_msgs::msg::Image>("raw_cv_image_topic", 10, std::bind(&DataLogger::save_cv_image, this, _1));
        // clusters_3d_subscriber_ = this->create_subscription<std::vector<std::shared_ptr<PointCloud>>>("lifecycle_chatter", 10, std::bind(&DataLogger::save_pointcloud, this, std::placeholders::_1));
    }
private:
    ...
    // std::shared_ptr<std::vector<std::shared_ptr<PointCloud>>> clusters_3d_subscriber_ = std::make_shared<std::vector<std::shared_ptr<PointCloud>>>();
    // std::shared_ptr<sensor_msgs::msg::Image> raw_cv_image_subscriber_= std::make_shared<sensor_msgs::msg::Image>();
