...
bool pointcloud::transformPointCloudMsg(
    const boost::shared_ptr<tf::TransformListener> &tf_listener,
    const std::string &target_frame, const sensor_msgs::PointCloud2 &cloud_in,
    sensor_msgs::PointCloud2 &cloud_out) {
  if (tf_listener) {
    try {
      ros::Time common_time;
      tf_listener->getLatestCommonTime(target_frame, cloud_in.header.frame_id,
                                       common_time, NULL);
      cloud_in.header.stamp = common_time;
      tf_listener->waitForTransform(target_frame, cloud_in.header.frame_id,
                                    ros::Time::now(), ros::Duration(1.0));
      pcl_ros::transformPointCloud(target_frame, cloud_in, cloud_out,
                                   *tf_listener);
...
bool pointcloud::transformPointCloud(
    const boost::shared_ptr<tf::TransformListener> &tf_listener,
...
bool pointcloud::transformPointCloud2(
    const boost::shared_ptr<tf::TransformListener> &tf_listener,
...
void pointcloud::getPointCloudROI(const sensor_msgs::RegionOfInterest &roi,
  ...
  if (roi.width + roi_size_adjustment < cloud_in->width)
    min_x = min_x + roi_size_adjustment;
  if (roi.height + roi_size_adjustment < cloud_in->height)
    min_y = min_y + roi_size_adjustment;
  ...
  for (size_t i = 0; i < pixel_loc.size(); i++) {
    ...
    if ((!pcl_isnan(pcl_point.x)) && (!pcl_isnan(pcl_point.y)) &&
        (!pcl_isnan(pcl_point.z)) && (!pcl_isnan(pcl_point.r)) &&
        (!pcl_isnan(pcl_point.g)) && (!pcl_isnan(pcl_point.b))) {
