...
void object::estimatePose(const BoundingBox &box,
  ...
  Eigen::Vector3f n2;
  Eigen::Vector3f n3 =
      (vertices[4] - vertices[0]) / (vertices[4] - vertices[0]).norm();
...
void object::estimatePose(const PointCloud::Ptr &xyz_input_cloud,
                          geometry_msgs::PoseStamped &pose, std::string shape,
...
void object::transformPose(
  ...
  if (tf_listener) {
    try {
      ...
      tf_listener->getLatestCommonTime(pose.header.frame_id, target_frame,
                                       common_time, NULL);
      pose.header.stamp = common_time;
      tf_listener->waitForTransform(target_frame, pose.header.frame_id,
                                    common_time, ros::Duration(0.1));
...
void object::get3DBoundingBox(
    const PointCloud::ConstPtr &cloud, const Eigen::Vector3f &normal,
...
void object::savePcd(const PointCloud::ConstPtr &pointcloud,
                     std::string log_dir, std::string obj_name) {
