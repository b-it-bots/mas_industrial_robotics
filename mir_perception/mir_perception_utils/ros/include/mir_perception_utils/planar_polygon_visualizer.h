...
{
 public:
  PlanarPolygonVisualizer(const std::string &topic_name, Color color, bool check_subscribers = true,
                          double thickness = 0.005);

  template <typename PointT>
  void publish(const pcl::PlanarPolygon<PointT> &polygon, const std::string &frame_id);
  ...
  void buildPolygonMarker(const typename pcl::PointCloud<PointT>::VectorType &points,
                          visualization_msgs::Marker &marker, const std::string &frame_id,
                          int id = 1);
