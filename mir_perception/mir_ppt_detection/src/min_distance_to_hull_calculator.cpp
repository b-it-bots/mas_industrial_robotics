...
void MinDistanceToHullCalculator::setConvexHullPointsAndEdges(const pcl::PointCloud<PointRGBA>::ConstPtr& convex_hull_cloud)
{   
    hull_points_.clear();
...
float MinDistanceToHullCalculator::computeMinDistanceToEdge(Eigen::Vector3f& pt, Eigen::Vector3f& edge_orig, Eigen::Vector3f& normalized_edge_vec)
{
    Eigen::Vector3f edge_orig_to_pt_vec = pt - edge_orig;
    return pow((pow(edge_orig_to_pt_vec.norm(), 2) - pow(normalized_edge_vec.dot(edge_orig_to_pt_vec), 2) ), 0.5);
}
...
float MinDistanceToHullCalculator::computeMinDistanceToHull(const PointRGBA& object_point)
{
    ...
    if (convex_hull_available_)
    {
        for( size_t i = 0; i < hull_points_.size(); i++){
            pt_to_edge_dist = computeMinDistanceToEdge(obj_pt, hull_points_[i], normalized_hull_edge_vecs_[i]);
...
float MinDistanceToHullCalculator::computeMinDistanceToHull(const pcl::PointCloud<PointRGBA>::ConstPtr& object_cloud)
{
