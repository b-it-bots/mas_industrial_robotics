...
class CavityRecognizer():
    ...
    def __init__(self, model_dir, model_name, debug_mode=False):
        ...
        self.pub_result = rospy.Publisher(
            "/mir_perception/cavity_finder/output/cavities_list", ObjectList, queue_size=1)
        self.sub_img = rospy.Subscriber(
            "/mir_perception/cavity_finder/output/cropped_cavities", ImageList, self.image_recognition_cb)
        self.pub_debug = rospy.Publisher(
            "/mir_perception/cavity_finder/output/classifier_debug_image", Image, queue_size=1)
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        object_recognizer = CavityRecognizer(
            model_dir=dataset_dir, model_name="output_graph.pb", debug_mode=True)
        rospy.sleep(0.025)
        rospy.spin()
