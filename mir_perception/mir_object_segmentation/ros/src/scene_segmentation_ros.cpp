...
SceneSegmentationROS::SceneSegmentationROS(double octree_resolution)
  ...
  cloud_accumulation_ =
      CloudAccumulation::UPtr(new CloudAccumulation(octree_resolution_));
...
void SceneSegmentationROS::segmentCloud(
  ...
  PointCloud::Ptr debug = scene_segmentation_->segmentScene(
      cloud, clusters, boxes, model_coefficients_, workspace_height_);
...
void SceneSegmentationROS::findPlane(const PointCloud::ConstPtr &cloud_in,
  ...
  cloud_debug = scene_segmentation_->findPlane(
      cloud_in, hull, plane, model_coefficients_, workspace_height_);
...
Eigen::Vector3f SceneSegmentationROS::getPlaneNormal() {
  Eigen::Vector3f normal(model_coefficients_->values[0],
                         model_coefficients_->values[1],
...
void SceneSegmentationROS::setVoxelGridParams(
    double voxel_leaf_size, std::string voxel_filter_field_name,
...
void SceneSegmentationROS::setPassthroughParams(
    bool enable_passthrough_filter, std::string passthrough_filter_field_name,
    double passthrough_filter_limit_min, double passthrough_filter_limit_max) {
...
void SceneSegmentationROS::setPrismParams(double prism_min_height,
...
void SceneSegmentationROS::setOutlierParams(double outlier_radius_search,
  ...
  scene_segmentation_->setOutlierParams(outlier_radius_search,
                                        outlier_min_neighbors);
}

void SceneSegmentationROS::setClusterParams(
  ...
  scene_segmentation_->setClusterParams(
      cluster_tolerance, cluster_min_size, cluster_max_size, cluster_min_height,
      cluster_max_height, cluster_max_length, cluster_min_distance_to_polygon);
}
