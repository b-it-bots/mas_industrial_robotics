...
void MultimodalObjectRecognitionROS::update()
{
    if (pointcloud_msg_received_count_ > 0 && image_msg_received_count_ > 0)
    {
        ...
        scene_segmentation_ros_->addCloudAccumulation(cloud_); //Exten this to work with multiple viewpoints
    ...
    if (!combined_object_list.objects.empty())
    {
        for (int i=0; i<combined_object_list.objects.size(); i++)
        {
                ...
                /* ->object_height_above_workspace_ - 0.05) */
            {
                ...
                //std::cout<<"base link to laser: "<<roi_base_link_to_laser_distance_<<std::endl;
                //std::cout<<"rgb max object pose to baselink: "<<roi_max_object_pose_x_to_base_link_<<std::endl;
                                                  ...
                                                  std::vector<PointCloud::Ptr> &clusters_2d)
{
    ...
    if (recognized_cloud_list_.objects.size() > 0)
    {
        ...
        for (int i=0; i<combined_object_list.objects.size(); i++)
        {
            ...
            {
                std::cout<<"[Cloud] Class: "<<combined_object_list.objects[i].name<<std::endl;
