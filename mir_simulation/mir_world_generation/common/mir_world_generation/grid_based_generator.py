...
class GridBasedGenerator(object):
    ...
    def __init__(self):
        ...
        self._wall_generation_threshold = config_data.get(
        ...
        self._exit_cell = (
            random.randint(0, self._num_of_rows - 1),
            self._num_of_cols - 1,
        )
    ...
    def create_occ_grid(self, border=100):
        ...
        for edge in self._walled_edges:
            ...
            self._draw_obj.line(
                GridBasedGenerator.offset_xy_with_border(xy, border), fill=0, width=4
            )
    ...
    def _make_connected(self, walled_edges):
        connected_nodes = Utils.get_connected_nodes(
            self._start_cell, walled_edges, self._num_of_rows, self._num_of_cols
        )
        ...
        for i in range(self._num_of_rows):
            for j in range(self._num_of_cols):
                for ws in self._grid[i][j].ws:
                    ...
                    ws_dict_list.append({"X": x, "Y": -y, "YAW": ws["theta"]})
        return ws_dict_list

    @staticmethod
    def offset_xy_with_border(xy, border):
        return [
            (xy[0][0] + border, xy[0][1] + border),
            (xy[1][0] + border, xy[1][1] + border),
        ]
