...
def get_3dmfv_per_batch(points_list, w, mu, sigma, normalize=True, whiten_points=True, mc=True, flatten=False):
    '''
    ...
    for points in points_list:
        ...
        #print ("dpi: ",d_pi.shape)
        #print ("dmu: ",d_mu.shape)
        #print ("dsigma: ",d_sigma.shape)
        fv = np.concatenate([d_pi, d_mu, d_sigma], axis=2)
        #print ("FV: ", fv.shape)
        fv = np.transpose(fv, axes=[0, 2, 1])
        ...
        if mc:
          ...
          mc_feature = features.calculate_feature_vector(points, False)
          mc_feature = np.reshape(mc_feature, (1,mc_feature.shape[0],1))
          print ("MC: ", mc_feature.shape)
          #print ("FV: ", fv.shape)
          mc_feature = np.tile(mc_feature, [5,mc_feature.shape[0],1])
          print ("MC: ", mc_feature.shape)
          #batch_mu = np.tile(np.expand_dims(mu, 0),[n_points, 1, 1]) 
          # add 5 feature from mc
          #fv = np.concatenate([fv, mc_feature, mc_feature, mc_feature, mc_feature, mc_feature], axis=2)
          print ("FV: ", fv.shape)
