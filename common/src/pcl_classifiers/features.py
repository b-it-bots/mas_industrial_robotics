...
def calculate_slices_description(pointcloud, number_of_slices):
    ...
    for k in xrange(number_of_slices):
        ...
        slices[k, slice.shape[0] + 1, :] = np.array([radius, outlier_error/inlier_error, radial_density])
...
def calculate_feature_vector(pointcloud, enable_color=False):
    ...
    for k in xrange(slices.shape[0]):
        slice = slices[k, :, :]
        slice = slice[np.where(np.sum(np.abs(slice),
                      axis=1) > 0)]
        if slice.shape[0] < 1:
