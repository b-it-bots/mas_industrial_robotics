...
class adjust_to_workspace(smach.State):
    ...
    def __init__(self, distance=None):
        ...
        self.ac_base_adj = actionlib.SimpleActionClient(
            self.ADJUST_SERVER, OrientToBaseAction
        )
...
class move_base_relative(smach.State):
    ...
    def __init__(self, offset=None):
        ...
        self.pub_relative_base_ctrl_command = rospy.Publisher(
            "/mcr_navigation/relative_base_controller/command", PoseStamped, latch=True
        )
        self.pub_relative_base_ctrl_event = rospy.Publisher(
            "/mcr_navigation/relative_base_controller/event_in", String, latch=True
        )
        self.sub_relative_base_ctrl_event = rospy.Subscriber(
            "/mcr_navigation/relative_base_controller/event_out",
            String,
            self.relative_base_controller_event_cb,
        )
        self.sub_collision_velocity_filter_event = rospy.Subscriber(
            "/mcr_navigation/collision_velocity_filter/event_out",
            String,
            self.collision_velocity_filter_event_cb,
        )
    ...
    def execute(self, userdata):
        ...
        if len(offset) == 3:
            ...
            relative_base_move.pose.position.x = self.sample_with_boundary(
                offset[0], offset[1]
            )
            relative_base_move.pose.position.y = self.sample_with_boundary(
                offset[2], offset[3]
            )

            q = tf.transformations.quaternion_from_euler(
                0, 0, self.sample_with_boundary(offset[4], offset[5])
            )
            relative_base_move.pose.orientation = Quaternion(*q)
        ...
        timeout = rospy.Duration.from_sec(
            10.0
        )  # wait for the done event max. 10 seconds
        ...
        while True:
            ...
            if (rospy.Time.now() - start_time) > timeout:
                rospy.logerr(
                    "timeout of %f seconds exceeded for relative base movement"
...
class approach_pose(smach.State):
    def __init__(self, pose_name=""):
        smach.State.__init__(
            self, outcomes=["succeeded", "failed"], input_keys=["base_pose_to_approach"]
        )
    ...
    def execute(self, userdata):
        ...
        if type(self.pose_name2) is str:
            ...
            rospy.logerr(
                "pose parameter <<%s>> is not a string, aborting...", self.pose_name2
            )
            return "failed"
