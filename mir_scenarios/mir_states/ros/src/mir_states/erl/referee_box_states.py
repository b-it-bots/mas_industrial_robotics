...
class get_benchmark_state(smach.State):
    ...
    def execute(self, userdata):
        ...
        if self.bm_state.state.data == atwork_ros_msgs.msg.BenchmarkState.RUNNING:
            if (
                self.bm_state.phase.data
                == atwork_ros_msgs.msg.BenchmarkState.CALIBRATION
            ):
                rospy.loginfo("STATE: running_calibration")
                return "running_calibration"
            elif (
                self.bm_state.phase.data
                == atwork_ros_msgs.msg.BenchmarkState.PREPARATION
            ):
                rospy.loginfo("STATE: running_preparation")
                return "running_preparation"
            elif (
                self.bm_state.phase.data == atwork_ros_msgs.msg.BenchmarkState.EXECUTION
            ):
...
class send_refbox_logging_status(smach.State):
    def __init__(self):
        smach.State.__init__(self, outcomes=["done"], input_keys=["logging_status"])

        self.logging_status_pub = rospy.Publisher(
            "/robot_example_ros/logging_status", atwork_ros_msgs.msg.LoggingStatus,
        )
...
class send_benchmark_feedback_fbm1(smach.State):
    def __init__(self, phase="exec"):
        ...
        self.benchmark_pub = rospy.Publisher(
            "/robot_example_ros/benchmark_feedback",
        ...
        self.obj_sub = rospy.Subscriber(
            ...
            mas_perception_msgs.msg.ObjectList,
            self.callback,
            queue_size=1,
        )
...
class send_benchmark_feedback_fbm2(smach.State):
    def __init__(self, phase="exec"):
        ...
        self.phase_to_term = phase
        self.obj_sub = rospy.Subscriber(
            "/mcr_perception/object_selector/output/object",
            mas_perception_msgs.msg.Object,
            self.callback,
            queue_size=1,
        )
        self.grasp_sub = rospy.Subscriber(
...
class send_benchmark_feedback_plate_drilling(smach.State):
    def __init__(self):
        smach.State.__init__(
            ...
            input_keys=["after_receiving", "after_drilling"],
        )
    ...
    def execute(self, userdata):
        if userdata.after_receiving is None and userdata.after_drilling is None:
            rospy.logwarn(
                "[send_benchmark_feedback_platedrilling] Invalid feedback data"
            )
            return "failure"
        msg = atwork_ros_msgs.msg.BenchmarkFeedback()
...
class send_benchmark_feedback_force_fitting(smach.State):
    def __init__(self):
        smach.State.__init__(
            self,
            outcomes=["done", "failure"],
            input_keys=["assembly_aid_tray_id", "container_id"],
        )
...
class conveyor_belt_command(smach.State):
    ...
    def __init__(self, command, timeout=10):
        ...
        self.conveyor_belt_command_pub = rospy.Publisher(
            "/robot_example_ros/conveyor_belt_command",
            atwork_ros_msgs.msg.TriggeredConveyorBeltCommand,
        )
        self.conveyor_belt_status_sub = rospy.Subscriber(
            "/robot_example_ros/conveyor_belt_status",
            atwork_ros_msgs.msg.TriggeredConveyorBeltStatus,
            self.status_cb,
        )
    ...
    def execute(self, userdata):
        ...
        while (
            self.status_msg.state.data
            != atwork_ros_msgs.msg.TriggeredConveyorBeltStatus.START
            or self.status_msg.cycle.data != msg.next_cycle.data
        ):
...
class drilling_machine_command(smach.State):
    ...
    def __init__(self, command, timeout=10):
        ...
        self.drilling_machine_command_pub = rospy.Publisher(
            "/robot_example_ros/drilling_machine_command",
            atwork_ros_msgs.msg.DrillingMachineCommand,
        )
        self.drilling_machine_status_sub = rospy.Subscriber(
            "/robot_example_ros/drill_machine_status",
            atwork_ros_msgs.msg.DrillingMachineStatus,
            self.status_cb,
        )
        self.timeout = rospy.Duration.from_sec(timeout)
        ...
        if self.command not in ["move_up", "move_down"]:
            rospy.logerr(
                "Possible drilling machine commands are 'move_up' or 'move_down'"
            )
...
class get_task_spec(smach.State):
    def __init__(self):
        smach.State.__init__(
            self, outcomes=["task_received", "timeout"], output_keys=["task_list"],
        )

        self.request_state_pub = rospy.Publisher(
            "/task_specification_transformer_node/event_in", std_msgs.msg.String,
        )
        self.state_sub = rospy.Subscriber(
            "/task_specification_transformer_node/task_specification",
            std_msgs.msg.String,
            self.task_spec_cb,
        )
