...
class select_object_to_place(smach.State):
    ...
    def __init__(self):
        ...
        self.object_pose_pub = rospy.Publisher(
            self.OBJECT_POSE_TOPIC, geometry_msgs.msg.PoseStamped
        )
        ...
        if len(userdata.found_cavities) == 0:
            ...
            for i in range(len(userdata.rear_platform_occupied_poses)):
                if userdata.rear_platform_occupied_poses[i].obj.name == look_for:
                    userdata.selected_object = userdata.rear_platform_occupied_poses[
                        i
                    ].obj
                    ...
                    print "Selected %s to place" % userdata.rear_platform_occupied_poses[
                        i
                    ].obj.name
...
class grasp_obj_for_hole_from_pltf(smach.State):
    ...
    def execute(self, userdata):
        ...
        for i in range(len(userdata.rear_platform_occupied_poses)):
            rospy.loginfo(
                "userdata.rear_platform_occupied_poses[i].obj.name: %s",
                userdata.rear_platform_occupied_poses[i].obj.name,
            )
            if (
                userdata.selected_object.name
                in userdata.rear_platform_occupied_poses[i].obj.name
            ):
